...
class LazySatelliteERA5Dataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            fill_value: NaN填充值
            use_aux: 是否加载辅助特征
            aux_channels: 辅助特征通道数 (1=仅landmask, 4=lat/lon/sza/landmask)
            transform: 数据增强函数
            cache_size: LRU缓存大小
        """
        super().__init__()
        ...
        print(f"  辅助通道数: {self.aux_channels}")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        aux_raw = raw['aux']
        if self.use_aux and aux_raw is not None and self.aux_channels < aux_raw.shape[0]:
            aux_raw = aux_raw[-self.aux_channels:, :, :]  # 取最后N个通道 (landmask在最后)
...
class InMemorySatelliteDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.aux_data is not None:
            ...
            if self.aux_channels < aux.shape[0]:
                aux = aux[-self.aux_channels:, :, :]
            sample['aux'] = torch.tensor(aux, dtype=self.dtype)
...
class AssimilationMetrics:
    ...
    def gradient_loss(
        ...
        # 展平通道维度
        pred_flat = pred.contiguous().view(B * C, 1, H, W)
        target_flat = target.contiguous().view(B * C, 1, H, W)
